...
from .const import (
    ...
    CONF_HAS_GREEN_BIN, CONF_COLLECTION_TIME, CONF_SENSOR_NAME,
    DEFAULT_SENSOR_NAME, DEFAULT_BASE_URL, DEFAULT_LIMIT,
...
async def async_setup_entry(
    ...
    name = config.get(CONF_SENSOR_NAME, DEFAULT_SENSOR_NAME)
...
class BneWasteCollectionSensor(BinarySensorEntity):
    def __init__(self, shared_data, name, icon, alert_hours, recycle_week):
        ...
        self._attrs = {}
        self._attr_unique_id = f"bne_wc_{shared_data._property_number}_{'recycle' if recycle_week else 'general'}"
...
class BneWasteCollectionKerbsideSensor(BinarySensorEntity):
    def __init__(self, shared_data, name, icon, alert_hours):
        ...
        self._alert_hours = alert_hours
        self._attrs = {}
        self._attr_unique_id = f"bne_wc_{shared_data._property_number}_kerbside"
...
class BneWasteCollectionData:
    ...
    def _get_collection_day(self):
        ...
        if days_ahead < 0:
            days_ahead += 7
        elif days_ahead == 0 and dt_util.as_local(dt_util.now()).time() >= self._collection_time:
            days_ahead = 7
